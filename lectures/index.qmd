...
- Cohort of patients
- Prediction time points $t_{ij}$ for patient $i$ and $j_{th}$ time point
- Binary outcome $Y(t_{ij})$ for each prediction time
<div style="overflow: auto;">
  <img src="../images/regular_inregular_time_patterns.PNG" alt="Image" style="margin-bottom: 15px; width: 30%;">
  <p>
    **Regularly or irregularly triggered?**.
  </p>
</div>
- Predictors
  - Baseline/static data $X_i$ for patient $i$ (e.g., sex, birthday, preexisting conditions)
  - Time dependent data $Z_i(t)$ for patient $i$ (e.g., blood pressure, weight, disease severity collected at time $t$)

Note: $t \neq t_{ij}$ will often happen in real-world data
<div style="overflow: auto;">
...
# Motivation
The motivation for predictive modeling for cancer patients is that predictive modeling is a powerful tool, ensuring patients receive the right Systemic Anti-Cancer Treatment (SACT) at the right time. Not only does this optimize survival rates, but it also potentially maximizes the quality of remaining life for patients. As technology and medical knowledge advance, the integration of such models will become more important in delivering patient-centered care. There is a fine line between when to advise a patient to receive SACT as it can decrease a patient’s life quality over time. The purpose of using SACT should be to make a patient able to recover and survive (Best-case scenario). It can cause a patient to suffer more in the remaining lifetime, which could mean that a patient's outcome would be worse with SACT than without (Worst-case scenario).
