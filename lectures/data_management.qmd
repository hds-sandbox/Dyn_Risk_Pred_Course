...
**Level of expertise for clinicians:** Another consideration regarding health data, which have to be accounted for, is consistency in some medical expertise, as there can be a large gap in experience among the doctors doing the examination. This affects the quality as more experienced doctors have a higher probability of giving the correct diagnosis while also offering the most appropriate treatment, compared to doctors with less experience.
...
REDCap have a range of tools which ease the process of managing clinical data while also providing high security for handling the data with REDCap. Some of these tools are:
...
The problem about data leaking is when some information of the unseen data is present in the seen data. A example of this could be a patient who get sick and then go to the hospital, where the person is registered.
...
However, even if k-fold cross validation is a powerful technique, it can still lead to overfitting. As the models is repeatedly trained on the data, the model starts to recordnize patterns of the training data. To overcome this challenge of potential overfitting another measure can be used, which is the [**nested Cross-validation**](https://machinelearningmastery.com/nested-cross-validation-for-machine-learning-with-python/).
...
The two tables fictional and used to show how rare occurring event can be collapsed together.

Another approach to handle categorical values is to transform them into binary variables which represent each category in a variable. This is to provide the model with additional information and it is a way for linear models to handle categorical variables, which they are normally unable to use as input if the categorize are nominal or represented as text. One hot encoding also helps in terms of the distance in categorical variables. A model may interpret a categorical value to have some type of ordering among the variables, but by splitting the categorize into separate binary features, the model can handle the categorize more efficiently. A example could be a gender variable:
...
However, there are some consideration about how the dimensions of a dataset can grow rapidly and lead to consequences like [Curse of dimensionality](https://www.geeksforgeeks.org/curse-of-dimensionality-in-machine-learning/). Despite this consideration, does One-Hot encoding remains as a popular and effective tool in the feature engineering toolbox.

**Outliers** is something which also calls for feature engineering. Some outliers are legitimate, but other outliers can there by mistake. It could be a BMI of 200 or a age of 150, which could be due to a typo when a clinicians had to journal a patient. Some outliers may be real, but it can be convenient to leave them out of the model if they deviate much. This is a case-by-base consideration. To find outliers it can be helpful to look at upper percentiles in the summary statistics and see if there is a unexpected distance from the mean to upper percentiles and if the values seem to me a product of a mistake, the model would be better with the upper percentile cut.
...
#### Summary
This segment of the course have provided a comprehensive insight into the process of gathering data, retrospective - and prospective studies. There have been provided a comprehensive overview of data management. The aim of this section was to provide a understanding of data and how to process the data before the actual modelling. In the next section will models be describes and a wide range of methods to choose.

But before continuing to the next section, please check the exercise below and try to solve it.

When the exercise is solved, the next section "Modelling" can be found in the top bar or by pressing this button:
