...
-   **Coding/Classifications:** Addressing variations in coding systems and classifications used across different time periods or data sources.
...
**Data** refers to the raw information, measurements, or observations collected as the primary subject of interest. Through analysis and interpretation can insight extracted from raw data. For instance, data regarding a patient's blood pressure would include the certain measurements in one column and the corresponding values in another column.
...
A clear understanding of these distinctions is significant for any data-driven work, regardless of the modeling approach used. Effectively leveraging the interplay between data, metadata, and annotations is central to derive the most amount insights from the collected information.
...
| Confirmation bias | Cognitive bias where researchers might unintentionally seek out or interpret data in ways that confirms their existing beliefs |
...
For more information about biases, please see this [comprehensive list about biases in research from University of Oxford.](https://catalogofbias.org/biases/)
...
Central to the Danish healthcare system is the DNPR, a repository for patient data used for analyzing disease patterns and outcomes. A primary classification employed is the [Sundhedsvæsenets Klassifikations System (SKS)](https://english.sundhedsdatastyrelsen.dk/health-data-and-registers/classifications) for treatment codes. a database of all the codes used for all treatments based on World Health Organization’s International Classification of Disease, currently ICD10.

<div style="overflow: auto;">
 <img src="../images/medinfodk.PNG" alt="Screenshot of SKS-browser" style="margin-bottom: 15px; width: 150px;">
</div>
...
- **LABKA:** For clinical laboratory data in Denmark will are stored the [The clinical laboratory information system (LABKA)](https://www.dovepress.com/article/download/6939) system. This source predominantly uses NPU (Nomenclature for Properties and Units) codes which are unique identification number for each single investigation, providing a terminology for identification of clinical laboratory test values.
...
With the continuous increasing amount of data being produced globally, a need exists for ensuring research data are findable, accessible, interoperable, and reusable. Principles for good data, metadata, and infrastructure practices are considered important for navigating all of the available information.

[In 2016, the FAIR principles were introduced in the scientific journal Nature](https://www.nature.com/articles/sdata201618). These principles represented a well-received approach to addressing data management challenges. The FAIR data principles provide a guide on how to navigate in all of this data while optimizing the usage of the data.
...
As a simple example, consider a requirement where data for patients aged between 18 and 100 years must be extracted from a table named TUMOUR_DATA. If data from all columns are needed, the symbol (*) is utilized. The corresponding SQL query is formulated as follows:
...
Different types of databases, and the choice of database is based on the use case and preference. A popular database type is the **relational database**, which consist of multiple tables with connections among them through foreign keys.
