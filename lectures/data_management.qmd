...
Data collection and management constitute the foundation of predictive modeling. These processes often present significant challenges including:

- Time-consuming data gathering
- Mismatched expectations about data quality and completeness
- Integration datasets containing varying measures, naming conventions, and granularity levels

This section addresses these challenges through three main topics:
...
When working with data management, it is important to distinct between data, metadata, and annotations. Data is the actual subject of interest as it is the raw information which is gathered and which can provide valuable insight. Most people know about data, but metadata and annotations are also critical to make vast amounts of data understandable and usable.

<div style="text-align: center;">
<img src="../images/data_meta_annotations.PNG" alt="Image" style="margin-bottom: 15px; width: 59%;">
</div>

Then there is the annotations which provide more intuition about the meaning of the ranges in the data.

These distinctions are important for dynamic medicine prediction as their interplay is what gets the most out of the data.

There is the raw data which consists of the actual measurements taken from patients, such as specific values like blood pressure readings.


Example of data about blood pressure:
...
Then there is inherent information about the data, which is called the `metadata`.

Example of metadata of blood pressure:
...
Data providers in the context of dynamic prediction models within healthcare in Denmark, a selection of major organisations who gatekeep databases containing sensitive health information about patients, which is the most valuable resources for the predictive modelling. A few examples of some of the major holders of health data in Denmark are:

[**Health Data Authority**](https://english.sundhedsdatastyrelsen.dk/health-data-and-registers)

<div style="overflow: auto;">
<img src="../images/dhda.png" alt="Image" style="margin-bottom: 15px; width: 30%;">
</div>
...
There are also institutions such as:
...
<div style="overflow: auto;">
<img src="../images/stat_dk.PNG" alt="Image" style="margin-bottom: 15px; width: 30%;">
</div>

It is not strictly reserved for healthcare but heavily revolves around society, workforce, education, culture, energy, etc. The organization has existed for [almost 200 years](https://www.dst.dk/en/OmDS/historie), so the data horizon in terms of time goes far back.
...
However, as there are many official places to find such data, there are also **other** alternative sources, such as:

- Your own collected data
- Local databases
- etc.

Before deciding which data source should be used for the dynamic prediction model, potential pitfalls about the data quality in terms of biases and limitations should be addressed, which is what the next section will cover in depth.
...
The biggest risk is proceeding with analysis without recognizing these biases and limitations. As the data involves expertise beyond the analytical scope, including relevant experts as guides is essential.

<div style="overflow: auto;">
<img src="../images/Dunning-Kruger_effect.PNG" alt="Image" style="margin-bottom: 15px; width: 70%;">
</div>
Information about the Dunning-Kruger effect can be found [here](https://en.wikipedia.org/wiki/Dunning%E2%80%93Kruger_effect)
...
REDCap has a range of tools which ease the process of managing clinical data while also providing high security for handling the data with REDCap. Some of these tools are:
...
Returning to the example of predicting when to provide SACT treatment of palliative cancer patients, in order to most efficiently build model applications, implementing a well-established framework is recommended. An advanced example of such a framework can be found here: [Tomašev et al. (2021) Use of deep learning to develop continuous-risk models for adverse event prediction from electronic health records](https://www.nature.com/articles/s41596-021-00513-5). This is a great example of an end-to-end dynamic prediction model.
...
Missing data should not only be seen as a negative feature of the dataset. Missing data can also contain some information which can be interpreted in an analysis or model.
...
Health and people are two dynamic factors as sickness can develop in many ways, whereas people are hard to predict as the ideal behavior and the real behavior are not the same.

Due to all the uncertainty regarding health and people, there is no regular pattern in contact with hospitals, therefore the need for dynamic models.
...
The data collection has been made on behalf of administration and to journal a patient, if we look at patient registries.
These registries were not created for the purpose of storing data for predictive modeling, as is the case for many other data sources.
...
##### Missing at random:
The data is missing due to an underlying systematic structure of the data. An example could be that a registry about depression may have surveys where a lot of the surveys have missing data.
...
As patients come at varying timepoints during the day and they go through different parts of the healthcare system, an analyst may consider transforming the data into something with a structured data format, which helps the prediction model process the data efficiently.
...
This means that a data analyst cannot make a model on some data and set the model in production, as this would lead to **overfitting** as the model would have great performance on the data it was trained on, but most likely not on the real data.
<div style="text-align: center;">
  <img src="../images/Data_split.PNG" alt="Image" style="margin-bottom: 15px; width: 75%;">
</div>
The splitting of the data varies, depending on the task. The most basic approach is to have a training set and validation set.
...
As models become more complex, hyperparameter tuning is becoming more important for the model. To properly evaluate the hyperparameters, the validation set is used to optimize the hyperparameters until some stopping criteria are met.
...
Feature engineering is a crucial step in the data management process, as it is the time where the data can be cleaned, where features can be selected, transformed into the best possible input, and where additional features based on the existing features can be created to support the model's understanding of feature scales or interactions.

One of the first things to do when the data for the prediction model is secured is to **identify useful features**.
The most efficient way to measure the importance of the features is to discuss with the clinicians, as they are better suited to provide insight into what they think is important for the specific case. This is especially important as some predictors may demand a lot of work. The clinicians become essential for this matter to state if predictors are worth keeping and if so, what would be the most appropriate strategy to repair the predictor. This means that another important factor for choosing features is the **Data quality** of the features. Some features with low quality would be better to remove if there is not an expected gain from keeping the feature.
...
Categorical variables can also cause problems, as there can be some categories with few observations. These few observations can make it hard for the model to find any meaningful relations while also causing the model to overfit on these cases, so the model learns patterns related to these few observations, which could be random as there is not enough information to confirm a relation. Furthermore, there can be some privacy issues as health data is sensitive data and the model cannot reveal anything about certain individuals. This is why it can be beneficial to collapse some of the categories into fewer and larger ones. An example could be some types of cancer:
...
Another approach to handle categorical values is to transform them into binary variables which represent each category in a variable. This is to provide the model with additional information and it is a way for linear models to handle categorical variables, which they are normally unable to use as input if the categories are nominal or represented as text. One-hot encoding also helps in terms of the distance in categorical variables. A model may interpret a categorical value to have some type of ordering among the variables, but by splitting the categories into separate binary features, the model can handle the categories more efficiently. An example could be a gender variable:
