...
Predictive modelling uses statistical or machine learning models to identify patterns in historical data and apply them to predict future events or outcomes. For this course, these models are used to estimate the changing risk of a binary clinical events over time to assist clinicians in deciding upon potential interventions. In mathematical terms, predictive modelling involves finding a function $f$ that accurately maps one or more predictor variables $x$ (also called independent/explanatory variables or features) to an outcome $y$ (also called a dependent variable), such that $y=f(x)$. Estimating an arbitrary functional relation $f$ is a complex, high-dimensional problem. Therefore, the function is typically parametrised by a finite-dimensional parameter vector $\beta$. This relationship can be expressed as:


$$y=f(x,\beta).$$

The parameter vector $\beta$ determines how the predictor variables $x$ are combined to make predictions. During model training, the algorithm “learns” the values of $\beta$ that make $f(x, \beta)$ best fit the observed data. In addition to the model parameters described by $\beta$, many models have *hyperparameters*, which are set control the model’s structure or learning process. Hyperparameters are typically chosen by assessing model performance on a validation set.
...
For further reading on classification and regression problems, seer: [James et al. 2024:15-27](https://www.statlearning.com/)
...
Based on the provided formulas from this course. Enter the number in the following boxes and check if you have reached the correct numbers.
...
The ROC curve provides a visual representation of a model's ability to discriminate classes by plotting the true positive rate (TPR), or sensitivity, against the false positive rate (FPR), which is equivalent to $1−Specificity$, as the classification threshold $c$ varies from 0 to 1 (Figure 5) . The ROC curve illustrates the trade-off between TPR and FPR: increasing the classification threshold reduces FPR but also lowers TPR. The maximum possible value for ROC AUC is 1.0, which represents a perfect classifier, while a random classifier follows the diagonal and yields a ROC AUC of 0.5 (Figure 5). Thus, a ROC AUC significantly above 0.5 indicates superior classification performance.

<div style="overflow: auto; text-align: center;">
   <img src="../images/ROC.png" alt="Image" style="margin-bottom: 15px; width: min(400px, 100%);">
 </div>

Figure 5: Illustration of a ROC curve and its corresponding ROC AUC. The diagonal line represents the performance of a random classifier. The red area is the AUC performance of a given classifier. Image credit [Vesteghem, C. (2022)](https://vbn.aau.dk/ws/portalfiles/portal/549539126/phd_CPEV_e_pdf.pdf)

The PR curve plots precision against sensitivity, also known as recall (Figure 6). It shows the trade-off between precision and recall: a higher classification threshold $c$ increases precision by reducing false positives but decreases recall by reducing false positives. The PR curve is particularly useful when a dataset contains class imbalance, where the positive class is significantly smaller than the negative class. It emphasises the model’s performance on identifying positive cases by using precision and recall. True negatives are not part of these calculations, which makes the PR curve more informative than the ROC curve when the negative class dominates. The PR curve for a model is often compared to that of a random classifier represented by a horizontal line (Figure 6). This baseline value of the PR AUC (often approximated by *average precision*) is the proportion of positive cases in the dataset. An ideal model would have a PR AUC of 1, which indicates a perfect score on both precision and recall.

<div style="overflow: auto; text-align: center;">
   <img src="../images/PR.png" alt="Image" style="margin-bottom: 15px; width: min(400px, 100%);">
 </div>
Figure 6: Illustration of PR curve and its corresponding PR AUC (here AP = average precision). The horizontal line illustrates the performance of a random classifier. Image credit [Vesteghem, C. (2022)]( https://vbn.aau.dk/ws/portalfiles/portal/549539126/phd_CPEV_e_pdf.pdf)
...
where $\sigma$ is the logistic function that transforms the linear combination of predictor variables into a probability ranging from 0 to 1 (Figure 7). It is given by
...
The relationship between the logistic function, a threshold (typically $c = 0.5$), and the decision boundary are illustrated in Figure 7 in case of a single predictor.

<div style="text-align: center;">
   <img src="../images/Log_reg_decision_boundary.PNG" alt="Image" style="margin-bottom: 15px; width: min(750px, 75%);">
 </div>
Figure 7: Illustration of the predicted probability of belonging to the positive class in a logistic regression with a single predictor $x_1$. Using a classification threshold of $c = 0.5$ (horizontal dotted line), the vertical dotted line marks the corresponding decision boundary in $x_1$-space, separating datapoints into predicted negative (green) and positive (red) classes.
...
In certain scenarios, a benefit can be gained by combining the regularization properties of ridge regression and LASSO. This can be obtained by using elastic net regularizationhttps://en.wikipedia.org/wiki/Elastic_net_regularization that also allows for the adjustment of the weighting of the two methods. The elastic net penalty is expressed as:
...
where $r_i$ is the proportion of observations of one class in node $i$.

During the model optimization process, the algorithm evaluates the quality of potential splits using this index. The decision tree algorithm evaluates the potential splits using the Gini index. The process is selected based on the weighted average of the Gini indices of the resulting child nodes. A split is chosen only if the total impurity of the two new nodes is lower than the impurity of the original parent node. By selecting the split that results in the lowest combined Gini index, nodes are created that are as pure as possible.
...
Another regularization method, referred to as pruning, involves establishing a minimum number of data points that must be present in each node.

In regression problems, the methodology is similar as for classification. However, node impurity is evaluated using metrics such as the mean squared error (or other suitable regression metrics) instead of the Gini index.
An example of a simple classification tree is shown in Figure 8.

<div style="text-align: center;">
   <img src="../images/Decision_trees_l4.PNG" alt="Image" style="margin-bottom: 15px; width: min(750px, 100%);">
   <div>
   **Note**: Decision nodes are <u>Blue</u> and the Leaf nodes are <u>Green</u>.
   </div>
 </div>

Figure 8: Example of a decision tree for binary classification (green/red). Starting at the root node, the data is split by age > 65, then further by weight and height, until four leaf nodes are reached. The black boxes, illustrating the leaf nodes, show the proportion of green observations in the specific node.
...
In general, *ensemble learning methods* combine multiple models to solve problems better than any single model could, thus merging several weak learners into one (hopefully) strong predictor. This is also known as ‘wisdom of the crowd’. The most popular ensemble learning method for decision trees are bagging, random forest, and gradient boosted decision trees.
 ...
 For further reading, see this ref: [James et al. 2014:331-341](https://www.statlearning.com/)
...
In gradient boosting, a new (typically shallow) tree is at each iteration fitted to the negative gradients of the specified loss function with respect to the current model’s predictions. Each tree therefore learns how to correct the remaining mistakes of the ensemble rather than predicting the target variable directly.
...
where $F_i$ represents the current ensemble model after $i$ iterations, $f_i$ denotes the tree fitted to the residuals of $F_i$, and $\delta$  is a shrinkage hyperparameter (also called learning rate) that determines how much influence the new tree has on the updated model.
...
tThe $q$th hidden layer has $k_q$ nodes. The activation of the first node in the first hidden layer, $A_1^{(1)}$, is then given by
...
where $B_{1, 1, 0}$ is the bias term and $B_{0,1,j}$ is the weight for the $j$th feature given in the input layer. Further, $g$ is the activation function, often the ReLU function, $\max(0,x)$ or the logistic function.
...
In general, for the $p$th node in the $q$th hidden layer with activation $A_p^{(q)}$, we can write this process as
...
where $B_{q,p,0}$ is the bias of node $p$ in layer $q$ and $B_{q,p,j}$ (for $j\geq 1$) is the weight connecting node $j$ in layer $q-1$ to node $p$ in layer $q$.

The activations from the last hidden layer are passed to the output layer. There, a final linear combination of the inputs is computed and then passed through an output activation function, such as the logistic function $\sigma$ in the case of a binary outcome, to produce the final prediction (denoted $Y_i$ in Figure 9).

<div style="text-align: center;">
   <img src="../images/neural_network.png" alt="Image" style="margin-bottom: 15px; width: min(600px, 80%);">
 </div>
Figure 9: An illustration of a deep feedforward neural network with three hidden layers ($L_1$, $L_2$, and $L_3$). The input layer consists of the data points $x_{ij}$ with $m$ features. The $q$th layer consists of $k_q$ nodes which are fully connected to all the nodes in the next layer. $B_1$, $B_2$, $B_3$, and $B_4$ denote the weight/bias matrices that defines the layer-to-layer transformations, $f$ the output activation function, and $Y_i$ the final prediction.
...
First, the weights of the network are initialised. Data is then passed through the network to produce predicted outcomes based on the initialised weights. The prediction error is then computed at the output layer, and during a process called *backpropagation* the derivate of this error is propagated backward through the hidden layers by partial derivatives to calculate the gradients of the loss function with respect to each parameter. Finally, the parameters are then updated in the direction of the gradient.
...
Bayesian optimisationhttps://medium.com/data-science/a-conceptual-explanation-of-bayesian-model-based-hyperparameter-optimization-for-machine-learning-b8172278050f  is a method for finding the best hyperparameters efficiently. Instead of trying every combination, it builds a model that predicts how well different hyperparameters will perform. The algorithm then chooses new hyperparameters to test based on this model, balancing exploration of unknown regions and exploitation of promising areas. After each evaluation, the model is updated with the results, gradually improving its predictions. This approach often finds good hyperparameters with fewer experiments than grid or random search. Figure 12 illustrates Bayesian optimization.

<div style="text-align: center;">
  <img src="../images/modelling/bayes_opt.png" alt="Image" style="margin-bottom: 15px; width: min(800px, 100%);">
</div>

Figure 12: Illustration of Bayesian optimisation. The top graph shows how an unknown loss function (solid line) is modelled by a surrogate function (the dashed line) which also provides a measure of the uncertainty of the surrogate function (the shaded area). The bottom graph shows the expected improvements as the surrogate models the unknown loss function over multiple iterations.  Image credit [Vesteghem, C. (2022)](https://vbn.aau.dk/ws/portalfiles/portal/549539126/phd_CPEV_e_pdf.pdf)



For further reading, see for example [Shahriari et al. (2015)](https://ieeexplore.ieee.org/document/7352306)
...
Cross-validation is a technique employed to assess how a model performs on unseen data that produces an unbiased estimate of model performance. Instead of only splitting the data once and evaluating model performance on a single test set, the process is repeated multiple times, with different parts of the data allocated for training and testing. After a number of repetitions, called *folds* (Figure 13), the performance across test sets is averaged to estimate the model's overall performance. If $K$ splits were performed, it is called $K$-fold cross-validation.
...
Figure 13: Illustration of 5-fold cross-validation. Visualise the process of using different parts of the dataset for test set across 5 splits of the dataset.
...
If the nested cross-validation confirms an acceptable generalizability, a final model can be fitted using a train/validation split on the entire dataset.

For further reading, see [Gary Colins (2024) Evaluation of clinical prediction models (part 1): from development to external validation](https://www.bmj.com/content/384/bmj-2023-074819)
...
Describe its intended use in in practice, considering the clinicians' limited time, their accessibility to the data, and the availability of the patient.
...
Describe the measurable impact om employing the prediction model, such as optimising resource usage and lowering mortality.
...
In this context, a dynamic risk prediction model is applied before each treatment cycle to aid in clinicians in evaluating of whether the patient is fit to receive the treatment, with the rule that treatment should generally be withheld if the model predicts a 30-day mortality risk above 50%.
...
For further reading, see [Richard D. Riley (2024) Evaluation of clinical prediction models (part 1): from development to external validation](https://www.bmj.com/content/384/bmj-2023-074820)
...
&nbsp;
 ...
 <p align="center">
  <a href="http://localhost:4707/lectures/implementation.html" style="background-color: #4266A1; color: #FFFFFF; padding: 30px 20px; text-decoration: none; border-radius: 5px;">
    Continue to
    Implementation
  </a>
</p>
