...
The $q$th hidden layer has $k_q$ nodes. The activation of the first node in the first hidden layer, $A_1^{(1)}$, is then given by
...
2. Describe its intended use in in practice, considering the clinicians' limited time, their accessibility to the data, and the availability of the patient.
...
5. Describe the measurable impact om employing the prediction model, such as optimising resource usage and lowering mortality.
