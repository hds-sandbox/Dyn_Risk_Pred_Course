...
<!-- </div> <!-- ved ikke om jeg kan lide denne figur, hvad er value, hvad er x. Value er a priori fordelinger af beta med hhv en normal og en dobbelt exponential prior. Figur 6.7 er måske mere sigende--> -->
...
To do this, the algorithm needs a sampling space for each of the hyperparameters, which can be customly designed, or some commands utilize predetermined distributions.

This handles dimensionality well, as the algorithm only trains as many models as you specify, and works relatively well, despite probably not finding the absolute best hyperparameters.
...
Note that calibration is not related to the accuracy of the model, and it is possible to have an accurate model without it being calibrated.
...
It does so by quantifying the closeness of the predictions ($p_i$) to the reality ($y_i$) by
$$
...
## Reference
Introduction to statistical learning, osv osv
