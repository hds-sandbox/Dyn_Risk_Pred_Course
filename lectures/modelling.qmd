...
Ensemble methods combine multiple models to solve problems better than any single model could. They work by merging several weak learners into one strong predictor.

### Bagging

Bagging (Bootstrap Aggregating) builds many independent models from random data samples and averages their predictions. Each model trains on different data subsets created through random sampling with replacement.
...
Random Forest extends bagging by adding another layer of randomness. Not only does it sample data points, but it also samples features when building each tree.
...
Unlike bagging methods that build independent models, boosting builds models sequentially. Each new model focuses on fixing the mistakes of combined previous models.
