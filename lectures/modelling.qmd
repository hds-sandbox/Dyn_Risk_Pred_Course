...
Regression involves predicting a continuous outcome. This task uses feature variables to train a model that predicts a target variable. Examples include predicting house prices based on relevant features or predicting blood pressure based on physiological measurements.

Classification involves predicting the category, or class, to which an instance belongs based on its feature variables. This results in a discrete value being predicted. Classification can be binary, involving two categories, or multiclass, involving more than two categories. For binary classification, the outcomes are often represented as positive/negative or true/false. These scenarios range from loan approval decisions to medical diagnoses (Hastie et al. 2001:9-11).

In healthcare applications, for instance, a model might be trained using a large amount of clean, high-quality patient data. This model can then be fed information (features) about a new patient, and a prediction or output is produced. If certain physiological values, such as blood pressure or body temperature, are rising, the model might suggest a potential illness.
