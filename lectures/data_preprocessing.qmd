...
Different summaries of data within the look back window can be used, for example:
...
Data splitting strategies can become more extensive depending on the model class. When a model involves *hyperparameters* (see the *Modelling* section), a typical approach is to consider a validation set alongside the training and test sets. The validation set is used to compare different hyperparameter configurations and select the best-performing one before evaluating the model on the test set. This helps prevent overfitting to the test set during hyperparameter tuning. Lastly, if a model requires *calibration* (see the *Modelling* section), a separate calibration set might also be used.
...
Table 1: Before collapsing infrequent values of a table showing the count of patients who experience primary symptoms.

*After Collapsing Infrequent Values*

<div style="overflow: auto; text-align: center;">
<img src="../images/table33.PNG" alt="Data Split Illustration" style="margin-bottom: 15px; width: 75%;">
</div>
Table 2: After collapsing infrequent values of a table showing the count of patients who experience primary symptoms.
...
Ref: [Sinan Ozdemir (2022)](https://www.manning.com/books/feature-engineering-bookcamp)
...
Ref: [Aurélien Géron (2019):65-67](https://www.oreilly.com/library/view/hands-on-machine-learning/9781492032632/)
...
Determine which patients goes into the training, validation, calibration, and test set(using, e.g., 80% training, 5% validation, 5% calibration, and 10% test). You can either add a variable indicating which set each observation belongs to or split your data into the four pieces.

Perform feature engineering, for example: a. Do we want to remove some predictors? (e.g., due to missingness or weird values) b. One-hot encoding of categorical predictors c. Construct feature for age at each time point d. How to handle missing values? Imputation*, presence variables, …? e. Cap and normalise numerical predictors** f. Summary variables (aggregating historical information)
...
&nbsp;
 ...
 <p align="center">
  <a href="http://localhost:4707/lectures/modelling.html" style="background-color: #4266A1; color: #FFFFFF; padding: 30px 20px; text-decoration: none; border-radius: 5px;">
    Continue to
    Modelling
  </a>
</p>
