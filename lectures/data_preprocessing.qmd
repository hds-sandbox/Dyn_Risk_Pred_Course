...
Data splitting strategies can become more extensive depending on the model class. When a model involves *hyperparameters* (see the [Modelling section](modelling.qmd)), a typical approach is to consider a validation set alongside the training and test sets. The validation set is used to compare different hyperparameter configurations and select the best-performing one before evaluating the model on the test set. This helps prevent overfitting to the test set during hyperparameter tuning. Lastly, if a model requires *calibration* (see the [Modelling section](modelling.qmd)), a separate calibration set might also be used.
...
4. Determine which patients goes into the training, validation, calibration, and test set(using, e.g., 80% training, 5% validation, 5% calibration, and 10% test). You can either add a variable indicating which set each observation belongs to or split your data into the four pieces.
