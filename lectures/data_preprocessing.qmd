...
Another example of a potential reliability issue is found in the **Danish Register of Causes of Death**. In one study, by [Helweg-Larsen (2011)](https://pubmed.ncbi.nlm.nih.gov/21775346/), focusing on this register, it was shown that a significant proportion of the recorded causes of death were inaccurate.
...
Data splitting strategies can become more extensive depending on the model class. When a model involves *hyperparameters* (see the [Modelling](modelling.qmd)), a typical approach is to consider a validation set alongside the training and test sets. The validation set is used to compare different hyperparameter configurations and select the best-performing one before evaluating the model on the test set. This helps prevent overfitting to the test set during hyperparameter tuning. Lastly, if a model requires *calibration* (see the [Modelling](modelling.qmd)), a separate calibration set might also be used.
...
Before collapsing infrequent values of a table showing the count of patients who experience primary symptoms.

<div style="overflow: auto; text-align: center;">
<img src="../images/table33.PNG" alt="Data Split Illustration" style="margin-bottom: 15px; width: 75%;">
</div>
After collapsing infrequent values of a table showing the count of patients who experience primary symptoms.
