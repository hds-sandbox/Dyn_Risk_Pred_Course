...
made possible by established data exchange standards such as FHIR [Fast Healthcare Interoperability Resources (FHIR)](https://www.healthit.gov/topic/standards-technology/standards/fhir). This standard streamlines the interaction with healthcare data globally. FHIR was briefly introduced under Data Sources and Data Acquisition and will be further elaborated here.
...
Define the base URL for the mock server: https://test/fhir

Construct the full URL to access the patient resource. By following the RESTful principles, this can be achieved by appending /Patient to the base URL.

Use the provided get() (get_r in the R mock server) method to request the URL

The mock server will return a JSON object. Parse the JSON object to extract the list of patient entries.

Iterate through the entries and extract the available variables

Organize this information into a dataframe. The file request_fhir.py or request_fhir.R should be in the same folder as this markdown file.
