...
get_r <- function(url) {
  ...
  path_part <- str_remove(url, fixed(BASE_URL))
  ...
  if (entity == 'Patient') {
    if (entity_id != 0) {
      ...
      patient_info <- patient_list %>% slice(1)
    ...
    for (i in 1:nrow(param_grid)) {
      ...
      patient_info <- patient_list_df %>% slice(1)
      ...
      snomed_info <- snomed_list_df %>% slice(1)
      ...
      tryCatch({
        if (snomed_info$source == 'blood_data' && entity == 'Observation') {
        ...
        } else if (snomed_info$source == 'baseline_data' && snomed_info$label == 'smoker' && entity == 'Observation') {
          ...
          valid_obs_source <- snomed_info$source %in% c('blood_data', 'diag_data', 'quest_data', 'baseline_data')
          valid_proc_source <- snomed_info$source == 'treat_data'
      ...
      if (!is.null(entries_temp_df) && nrow(entries_temp_df) > 0) {
        ...
        if(entity == 'Observation') {
          entries_temp_df$value <- as.numeric(entries_temp_df$value)
        } else { # Procedure - maybe keep as character?
          entries_temp_df$value <- as.character(entries_temp_df$value)
        }
    ...
    if (length(entries_list_df) == 0) {
      ...
      if (nrow(entries_df) == 0) {
        ...
        entry_strings <- purrr::pmap_chr(entries_df, function(...) {
          entry_info <- list(...)
    ...
    if (!is.null(query_params) && length(query_params) > 0) {
      query_string_parts <- mapply(function(name, value) {
        paste0(utils::URLencode(name), "=", utils::URLencode(value))
